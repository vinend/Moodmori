...
class MoodLogController {
  ...
  try {
    const moodLogs = await moodLogRepository.getPublicMoodLogs();
    if (!moodLogs) {
      return responseFormatter.error(res, 'No public mood logs found', 404);
    }
    responseFormatter.success(res, { moodLogs });
  } catch (error) {
  ...
  async createPublicMoodLog(req, res) {
  try {
    const userId = req.user.id;
    const { moodId, note, logDate } = req.body;
    ...
    // Verifikasi mood ada di database
    const mood = await moodRepository.getMoodById(moodId);
    if (!mood) {
      return responseFormatter.error(res, 'Invalid mood selected', 400);
    }
    ...
    // Mendapatkan mood log yang baru saja dibuat
    const createdMoodLog = await moodLogRepository.getMoodLogById(newMoodLog.id);
    ...
    console.error('Error in createPublicMoodLog:', error);
    responseFormatter.error(res, 'Failed to create mood log', 500);
  }
}
}
